...
import { r as hero_nepal_default, t as AnimatedButton } from "./hero-nepal-CptWjQCF.mjs";
...
import { n as fetchApi, t as DetailModal } from "./apiClient-Bovnh8EF.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/volunteers-mGpNKTpZ.js
...
function mapVolunteer(volunteer) {
	...
	return {
		...
		...volunteer.gallery?.[0] ? { profileImage: volunteer.gallery[0] } : {},
		location: address,
	...
	try {
		...
		return dataset.filter((item) => {
			...
			if (filters.skill && filters.skill !== "all") {
				if (!item.skills.some((skill) => skill.toLowerCase().includes(filters.skill.toLowerCase()))) return false;
			}
			if (filters.interest && filters.interest !== "all") {
				if (!item.interests.some((interest) => interest.toLowerCase().includes(filters.interest.toLowerCase()))) return false;
			}
          ...
          ${item.district}
          ${item.province}
        `.toLowerCase().includes(filters.location.toLowerCase())) return false;
			}
			if (filters.availability && filters.availability !== "all") {
				if (!item.availability.toLowerCase().includes(filters.availability.toLowerCase())) return false;
			}
			if (filters.experience && filters.experience !== "all") {
				if (!item.experience.toLowerCase().includes(filters.experience.toLowerCase())) return false;
			}
			if (filters.query) {
        ...
        `.toLowerCase().includes(q);
				const matchSkills = item.skills.some((skill) => skill.toLowerCase().includes(q));
				const matchInterests = item.interests.some((interest) => interest.toLowerCase().includes(q));
				return matchName || matchBio || matchLocation || matchSkills || matchInterests;
			}
			return true;
		});
	} catch (error) {
